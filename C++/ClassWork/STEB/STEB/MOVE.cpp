...
TCHAR szClassWindow[] = TEXT("ДВИГАЙСЯ"); /* Имя класса окна */
...
LRESULT CALLBACK KeyboardHanldler(HWND hWnd, UINT uMessage, WPARAM wParam, LPARAM lParam) {
	if (wParam == VK_LEFT) vx = -10;
	if (wParam == VK_RIGHT) vx = 10;
	if (wParam == VK_UP) vy = -10;
	if (wParam == VK_DOWN) vy = 10;
	return 0;
}
